...
openssl x509 -outform DER -in $CA_HOME/temp/vpn.client.pem.crt -out $CA_HOME/temp/vpn.client.der.crt

cat $CA_HOME/certs/$CA_NAME.pem.crt $ROOT_CA_HOME/certs/$ROOT_CA_NAME.pem.crt > $CA_HOME/temp/ca.chain.pem.crt
openssl pkcs12 -export -name "VPN client certificate" -inkey $CA_HOME/temp/vpn.client.pem.key -in $CA_HOME/temp/vpn.client.pem.crt -certfile $CA_HOME/temp/ca.chain.pem.crt -out $CA_HOME/temp/vpn.client.p12 -password pass:P@ssw0rd
