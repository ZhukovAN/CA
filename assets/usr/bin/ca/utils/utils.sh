...
# Create folders for managed CAs
initializeCaFolder() {
...
# Initialize managed CA certificate and CRL serials with random values
...
initializeRootCa() {
    ...
    openssl req -new \
      -config ${ROOT_CA_CONF}/ca.conf \
      -out ${ROOT_CA_DATA}/csr.pem \
      -key ${ROOT_CA_DATA}/private/key.pem
    openssl ca -batch \
      -selfsign \
      -config ${ROOT_CA_CONF}/ca.conf \
      -in ${ROOT_CA_DATA}/csr.pem \
      -out ${ROOT_CA_DATA}/certs/ca.pem \
...
# Initialize intermediate managed CA
initializeIntermediateCa() {
    ...
    echo "Generate and sign CA certificate signing request"
    openssl req -new \
      -config ${INTERMEDIATE_CA_CONF}/ca.conf \
      -out ${INTERMEDIATE_CA_DATA}/csr.pem \
      -key ${INTERMEDIATE_CA_DATA}/private/key.pem
    openssl ca -batch \
      -config ${ROOT_CA_CONF}/ca.conf \
      -in ${INTERMEDIATE_CA_DATA}/csr.pem \
      -out ${INTERMEDIATE_CA_DATA}/certs/ca.pem \
      -extensions signing_ca_ext \
      -policy extern_pol -notext
...
generateSslServerCertificate() {
    ...
    mkdir -p ${OUT} && mv ${TMP}/* ${OUT}/ && rm -rf ${TMP}
...
generateIntermediateCaCertificate() {
    ...
    openssl ca -batch \
      ...
      -policy extern_pol \
      -notext
    SERIAL=`openssl x509 -in ${TMP}/ca.pem -serial -noout | sed -r 's/serial=//g'`
    OUT=${ROOT_CA_DATA}/out/${SERIAL}
    # Don't know how to avoid OpenSSL putting signed certificate copy named ${OUT}.pem, so just remove it
    rm -rf ${OUT}.pem
    ...
    echo "Copy certificate generation artifacts to ${OUT} folder"
    mkdir -p ${OUT} && mv ${TMP}/* ${OUT}/ && rm -rf ${TMP}
    openssl x509 -outform DER -in ${OUT}/ca.pem -out ${OUT}/ca.crt    
}
